...
public class MathScoreStatisticRepository(ApplicationDbContext dbContext) : IScoreStatisticRepository
{
    ...
    public async Task<List<int>> GetStatisticTotalStudentsAsync()
    {
        var result = await _dbContext.Students
            ...
            .Where(s => s.MathScore != null)
            .GroupBy(s =>
                s.MathScore < 4 ? "Level 1" :
                s.MathScore < 6 ? "Level 2" :
                s.MathScore < 8 ? "Level 3" :
                "Level 4"
            )
            .Select(g => g.Count())
            .ToListAsync();
